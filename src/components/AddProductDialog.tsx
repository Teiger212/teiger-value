...
interface AddProductDialogProps {
  onAddProduct: (product: { name: string; imageUrl?: string; description: string; price: number }) => void;
}

export const AddProductDialog: React.FC<AddProductDialogProps> = ({ onAddProduct }) => {
  const [newProduct, setNewProduct] = useState({ name: "", imageUrl: "", description: "", price: 0 });
  const [isValid, setIsValid] = useState(false);


  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    const updatedProduct = { ...newProduct, [name]: name === 'price' ? parseFloat(value) : value };
  ...
  const handleAdd = () => {
    ...
    if (validationResult.success) {
      ...
      setNewProduct({ name: "", imageUrl: "", description: "", price: 0 });
      close();
    }
  };
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <Button onClick={handleAdd} disabled={!isValid}>Add Product</Button>
      </DialogContent>
    </Dialog>
  );
};